...
> 共 16 个插件，分 7 类。`category` 字段记录在 `.claude-plugin/marketplace.json`，以下小标题与之一一对应。
...
> 一对互补的处理：read-digest 管 **input 侧**（读懂别人/AI 丢来的稿），fair-copy 管 **output 侧**（清自己反复改过的稿）。
...
| fair-copy | 把自己反复用 AI 改过、混进过程痕迹的工作稿，清成读者第一次看就是成品的交付稿（誊清稿）：剥掉修订历史 / 版本说明 / 行内批注 / 旁批 / 备选写法，严格保留正文，拿不准先问再删（区别于「去 AI 味」） | `/plugins install fair-copy@aipm-tools` |
