...
> 三件互补的处理：read-digest 管 **input 侧**（读懂别人/AI 丢来的稿）；fair-copy 管 **output 侧 · 过程痕迹**（清自己反复改过的稿）；humanizer 管 **output 侧 · 文风**（去 AI 味）。fair-copy 与 humanizer 常先后用：先清痕迹，再去味。
...
| fair-copy | 把自己反复用 AI 改过、混进过程痕迹的工作稿，清成读者第一次看就是成品的交付稿（誊清稿）：剥掉修订历史 / 版本说明 / 行内批注 / 旁批 / 备选写法，严格保留正文，拿不准先问再删（去 AI 味用 humanizer） | `/plugins install fair-copy@aipm-tools` |
| humanizer | 去 AI 味（降 AIGC 率）：把一眼就是机器写的文本改回像人写的。中文为主——套话开场 / 空洞收尾 / 互联网黑话（赋能 / 抓手 / 闭环）/ 全角破折号滥用 / 不仅…而且 / 强行排比 / 客服腔 / 模糊归因，附英文写作专有 tell；靠 cluster 判断不见词就改，去味之余保人味（区别于 fair-copy 的去过程痕迹）。译改自 blader/humanizer (MIT) | `/plugins install humanizer@aipm-tools` |
