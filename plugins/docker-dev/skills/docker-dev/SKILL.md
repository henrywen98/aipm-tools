...
name: docker-dev
description: >-
  Docker 与容器开发助手：Dockerfile 优化、docker-compose 编排、多阶段构建与容器安全加固。
  Use when the user says "优化 Dockerfile", "Docker 镜像太大", "docker-compose 配置",
  "容器安全审计", "多阶段构建", "optimize Dockerfile", "compose", "dockerfile best practices",
  "container security", or wants to reduce image size / improve layer caching / harden
  container runtime. Covers build performance, secret management, and production-ready
  container patterns.

  Not for: Kubernetes manifests (use a K8s-specific skill), cloud provider ECS/Fargate/Cloud Run
  deployment SDKs, general Linux sysadmin.
license: MIT
metadata:
  version: 1.0.0
  upstream_author: Alireza Rezvani
  upstream_source: https://github.com/alirezarezvani/claude-skills/tree/main/engineering/docker-development
  upstream_snapshot: 2026-04-08
...
通过 aipm-tools marketplace 安装：

```bash
/plugins install docker-dev@aipm-tools
```

本 skill 改编自 alirezarezvani/claude-skills (MIT License)，详见插件根目录 `NOTICE.md`。

---

## Related Skills

- **cicd-dev** — CI/CD 流水线构建。互补关系：docker-dev 负责构建容器镜像，cicd-dev 负责自动生成 GitHub Actions / GitLab CI 流水线把这些镜像推送到注册表并部署。
